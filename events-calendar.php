...
// Calendar nav links — no inline onclick needed; clear day selection
    document.querySelectorAll('.cal-nav a').forEach(a => {
...
// Calendar cell click
    ...
    /* ── Submit Event Modal ── */
    (function() {
      var modal   = document.getElementById('submitEventModal');
      var btn     = document.querySelector('.add-event-btn');
      var closeEl = document.getElementById('closeEventModal');
      var form    = document.getElementById('submitEventForm');
      var submitBtn;
      if (!modal || !btn) return;
      ...
      function openModal()  {
        ...
        // Clear stale feedback on re-open
        var err  = modal.querySelector('.form-error');
        var succ = modal.querySelector('.form-success');
        if (err)  err.remove();
        if (succ) succ.remove();
        // Re-bind submit button reference (may have been removed/recreated)
        submitBtn = form.querySelector('button[type=submit]');
        if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Publish Event'; }
      }
      function closeModal() {
      ...
      // Prevent double-submit: disable button on form submit, re-enable on modal close
