...
if ($currentRole !== 'DIRECTOR ICT') {
    header('Location: index.php');
    exit;
}

$pageTitle = 'ICT Management Portal';

// ── Director Personal Info ──
$directorUser = $currentUser;
$directorRow = db()->fetch("
    ...
    LEFT JOIN council_members cm ON cm.user_id = u.id AND cm.position = 'DIRECTOR ICT'
    LEFT JOIN media m ON cm.profile_image_id = m.id
    WHERE u.id = ?
    LIMIT 1
", [$directorUser['id']]);

$fullName   = htmlspecialchars(($directorRow['first_name'] ?? '') . ' ' . ($directorRow['last_name'] ?? ''));
$email      = htmlspecialchars($directorRow['email'] ?? '');
$studentId  = htmlspecialchars($directorRow['student_id'] ?? '—');
$phone      = htmlspecialchars($directorRow['phone'] ?? '—');
$position   = htmlspecialchars($directorRow['position'] ?? 'Director of ICT');
$termStart  = $directorRow['term_start'] ?? '';
$termEnd    = $directorRow['term_end'] ?? '';
$profileImg = $directorRow['profile_image_path'] ?? null;
$termActive = (bool)($directorRow['term_active'] ?? 0);
...
// ── Open Complaints (assigned to this director) ──
$myComplaints = db()->fetch("SELECT COUNT(*) as c FROM complaints WHERE status = 'OPEN' AND assigned_to = ?", [$currentUser['id']])['c'] ?? 0;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="dashboard-header-section">
                        ...
                        <p class="dashboard-subtitle">
                            <?php echo $termLabel ? 'Term: ' . htmlspecialchars($termLabel) : 'ICT Management &amp; Systems Oversight'; ?>
                        </p>
                    </div>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                         ...
                         <div class="stat-card">
                             ...
                             <div class="stat-card-value"><?php echo (int)$publishedNews; ?></div>
                            ...
                            <div style="display:flex;gap:16px;align-items:center;margin-bottom:20px;padding-bottom:20px;border-bottom:1px solid rgba(201,168,76,0.1);">
                                ...
                                <?php else: ?>
                                    <div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,#C9A84C,#E8C97A);display:flex;align-items:center;justify-content:center;color:#fff;font-size:28px;font-weight:700;flex-shrink:0;">
                                        <?php echo strtoupper(substr($fullName ?: 'DJ', 0, 1)); ?>
                                    </div>
                                <?php endif; ?>
                                <div>
                                    <div style="font-size:17px;font-weight:600;color:#C9A84C;"><?php echo $fullName ?: 'Director ICT'; ?></div>
                    ...
                    <div class="table-container" style="margin-bottom:24px;">
                        <div style="padding:16px 20px;border-bottom:1px solid rgba(138,155,184,0.1);display:flex;justify-content:space-between;align-items:center;">
                            <h3 style="margin:0;font-size:15px;font-weight:600;color:#3f3e3d;"><i class="bi bi-file-earmark-text" style="color:#C9A84C;"></i> Recent Resolutions &amp; Motions</h3>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            <div class="stat-card-header">
                                <span class="stat-card-label">Assigned Complaints</span>
                                <div class="stat-card-icon warning">
                                    <i class="bi bi-clipboard-check"></i>
                                </div>
                            </div>
                            <div class="stat-card-value"><?php echo (int)$myComplaints; ?></div>
                            <div class="stat-card-label">Open complaints assigned to me</div>
                        </div>
                        <a href="ga-sessions.php" class="text-decoration-none">
                        <div class="stat-card" style="cursor:pointer;">
                            <div class="stat-card-header">
                                <span class="stat-card-label">Managing GA</span>
                                <div class="stat-card-icon info">
